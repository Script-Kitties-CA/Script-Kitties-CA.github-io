  ...
  return (
    <div className="bg-violet-250 dark:bg-violet-300 rounded-lg shadow-lg my-4 p-3 transition duration-300 ease-out transform hover:scale-102">
      <div class="flex h-3 w-3 mb-1">
        <span
          class={`animate-ping absolute inline-flex h-3 w-3 rounded-full bg-emerald-500 opacity-75 ${
            props.started ? "visible" : "hidden"
          }`}
        ></span>
        <span
          class={`relative inline-flex rounded-full h-3 w-3 ${
            props.started ? "bg-emerald-300" : "bg-gray-400 dark:bg-gray-500"
          }`}
        ></span>
      </div>
      <div className="text-white">
        ...
        <p className="text-xl">
          {getTimeString(startTime, endTime, props.started)}
        </p>
      </div>
    </div>
  );
